...
class CodonTransformer(nn.Module):
    def __init__(self, vocab_size_target, vocab_size_input, d_model, nhead, num_encoder_layers, num_decoder_layers, dim_feedforward, dropout):
        ...
        # self.fc_out_3 = nn.Linear(int(d_model/2), int(d_model/4))
        # self.fc_out_4 = nn.Linear(int(d_model/4), vocab_size_target)
        self.act = nn.ReLU()
        
        self.d_model = d_model

    def forward(self, tgt, src, memory_calm=None, src_key_padding_mask=None, tgt_key_padding_mask=None):
        ...
        # output_codon = self.fc_out_4(self.act(self.fc_out_3(self.act(self.fc_out_2(self.act(self.fc_out_1(output)))))))
