    ...
    for gene_id in gene_ids:
        ...
        while retries < max_retries:
            try:
                ...
                print(f"Error fetching ortholog group for {gene_id}: {e}")
                retries += 1
                if retries < max_retries:
                    print(f"Retrying in 5 seconds... (Attempt {retries + 1})")
                    time.sleep(5)
                else:
                    print(f"Failed to fetch ortholog group for {gene_id} after {max_retries} attempts. Skipping.")
                    continue
        ...
        if ortholog_groups:
            ...
            try:
                ...
                sequence_only = "".join(line for line in cds_sequence.split("\n") if line and not line.startswith(">"))
            except Exception as e:
                print(f"Error fetching CDS sequence for {gene_id}: {e}")
                continue

            try:
                cds_aa_sequence = REST.kegg_get(gene_id, "aaseq").read()
            except Exception as e:
                print(f"Error fetching CDS sequence for {gene_id}: {e}")
                continue
